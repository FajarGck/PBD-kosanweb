...
<body>
	...
	<div class="container">
		<h2	h2	h2 class="text-center m-0 pb-0 px-4 pt-2 shadow-lg pb-2 mx-auto rounded" style="max-width: fit-content;">TAMBAH DATA KAMAR</h2>
		<br/>
		...
		<br/>
		<br/>
		<form method="post" action="tambahkamar_aksi.php" class="row g-3">
		<div class="col-md-4">
			<label for="input-idkamar" class="form-label">ID Kamar</label>
			<input type="text" class="form-control" id="input-idkamar" name="idkamar" value="kmr-1" required>
		</div>
		<div class="col-md-4">
			<label for="input-nama" class="form-label">NAMA KAMAR</label>
			<input type="text" class="form-control" id="input-nama" name="namakamar" value="kmr-1A" required>
		</div>
		<div class="col-md-4">
		...
		<div class="col-md-4">
			<?php 
			include 'koneksi.php';
			$idpemilik = mysqli_query($koneksi, 'SELECT IdPemilik FROM pemilik');
			?>
			<label for="input-idpemilik" class="form-label">ID PEMILIK</label>
			<select class="form-select" id="input-idpemilik" name="idpemilik" required>
				<option selected disabled>Pemilik yang Ada</option>
				<?php 
				while($d = mysqli_fetch_assoc($idpemilik)) {
				?>
					<option value="<?php echo $d['IdPemilik']; ?>"><?php echo $d['IdPemilik']; ?></option>
				<?php 
				}
				?>
			</select>
